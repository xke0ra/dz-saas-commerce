...
| Inventory | inventory_manual_adjustment | Tenant owner/admin/system | yes | implemented | inventory item id, product id, tenant id, stock movement id, quantity delta, reserved delta, reason; PII/secrets not allowed | adjust stock and assert movement plus AuditLog |
...
| Security | 2FA enable/disable/recovery regeneration/challenge | Admin/support/vendor user | yes | implemented for enable, disable, recovery-code regeneration, and successful challenge | actor, target user, panel id, challenge method only; never TOTP/recovery/secret | enable 2FA, regenerate codes, pass challenge, disable 2FA, assert audit without raw codes |
