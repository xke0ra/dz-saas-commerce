...
المرحلة القادمة يجب أن تضيف manual inventory adjustment مع AuditLog، وأن تصمم product variants قبل التنفيذ، لأنهما سيؤثران على checkout والمخزون والطلبات.
...
- manual inventory adjustment مع AuditLog.
...
- الحالة الحالية: inventory item لكل منتج، reservation/release/settle، وأساس stock movement ledger، وquick checkout reservation يكتب `reserved` movements، وrelease يكتب `released` movements، وsettlement يكتب `settled` movements، وreturn restock يكتب `restocked` movements.
- المطلوب: manual inventory adjustment مع AuditLog، low stock alerts، variant inventory.
- الأولوية: P1.
