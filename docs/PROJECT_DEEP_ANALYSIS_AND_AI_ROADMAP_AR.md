...
المرحلة القادمة يجب أن تكمل ربط stock movement ledger بالـ restock/returns بحذر، وأن تصمم product variants قبل التنفيذ، لأنهما سيؤثران على checkout والمخزون والطلبات.
...
- stock movement ledger restock/returns integrations.
...
- product variants/options.
- stock movements ledger restock/returns integrations.
- product import/export.
- bulk order operations.
- order timeline.
- checkout UX.
...
- الحالة الحالية: inventory item لكل منتج، reservation/release/settle، وأساس stock movement ledger، وquick checkout reservation يكتب `reserved` movements، وrelease يكتب `released` movements، وsettlement يكتب `settled` movements.
- المطلوب: ربط restock/returns بالـ stock movements، low stock alerts، variant inventory.
- الأولوية: P1.
