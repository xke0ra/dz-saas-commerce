...
Checklist تنفيذية قصيرة قبل تشغيل real staging smoke. لا تستبدل هذه الوثيقة runbooks الموجودة؛ استخدمها كبوابة قبل `deploy/staging/staging-smoke.sh`، وراجع `docs/STAGING_DEPLOYMENT_RUNBOOK_AR.md` للخطوات التفصيلية. هذه checklist لا تعني أن staging منشور الآن.
...
- [ ] VPS/provider أو منصة استضافة محددة. الحالة الحالية: pending حتى يتم توفير VPS/provider.
- [ ] staging domain أو subdomain أو temporary hostname محدد للـ storefront. الحالة الحالية: pending حتى يتم توفير domain/hostname.
...
- [ ] real staging environment يبقى pending إذا لم يتوفر VPS/provider وdomain/hostname بعد.
